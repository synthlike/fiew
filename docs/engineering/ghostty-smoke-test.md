...
14. Create labelled and unlabelled source bookmarks with `Space b n`, then create one from a diff line. Open `Space b Enter`; verify keyboard and mouse preview, `Enter`, `[ b`/`] b`, confirmed deletion, and restart persistence. Confirm `.bookmarks/bookmarks.json` is created without source or Git mutation by Skaut.
...
16. Use `Space q`. Confirm Skaut exits, the previous terminal contents return, the cursor is visible, and normal line input and echo work.
...
18. Run Skaut again and press `Ctrl-C`. Confirm the same restoration behavior.
